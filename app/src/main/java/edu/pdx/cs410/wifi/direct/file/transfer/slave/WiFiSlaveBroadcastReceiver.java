...
public class WiFiSlaveBroadcastReceiver extends BroadcastReceiver {

    private WifiP2pManager manager;
    private Channel channel;
    private SlaveActivity activity;

    public WiFiSlaveBroadcastReceiver(WifiP2pManager manager, Channel channel, SlaveActivity activity) {
        super();
        this.manager = manager;
        this.channel = channel;
        this.activity = activity;

        activity.setSlaveStatus("Server Broadcast Receiver created");
    }
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (WifiP2pManager.WIFI_P2P_STATE_CHANGED_ACTION.equals(action)) {
            ...
            if (state == WifiP2pManager.WIFI_P2P_STATE_ENABLED) {
                activity.setSlaveWiFiStatus("Wifi Direct is enabled");
            } else {
                activity.setSlaveWiFiStatus("Wifi Direct is not enabled");
            }
...
//                activity.masterIp = wifiInfo.groupOwnerAddress.toString();
                if (!wifiInfo.isGroupOwner) {
                    try {
                        ...
                        activity.setSlaveExceptionStatus("Exception:"+ e.toString());
                    }
                }
                else{
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case 2:
                    ...
                    activity.setSlaveExceptionStatus(str_msg);
                    break;
                default:
